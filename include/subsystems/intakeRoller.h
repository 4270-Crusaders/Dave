...
public:
	 ...
	 *
	 * @param left_intake_motor
	 * @param right_intake_motor
	 */
	explicit Intake(pros::Motor left_intake_motor, pros::Motor right_intake_motor)
		: leftIntakeMotor(std::move(left_intake_motor)),
		  rightIntakeMotor(std::move(right_intake_motor)) {
	}
	...
	void periodic() override {
		// EX: debugging tasks
		std::cout << "Left Intake speed: " << leftIntakeMotor.get_actual_velocity() << std::endl;
		std::cout << "Right Intake speed: " << rightIntakeMotor.get_actual_velocity() << std::endl;
	...
	void setPct(const double pct) {
		...
		this->leftIntakeMotor.move_voltage(static_cast<std::int32_t>(pct * 12000.0));
		this->rightIntakeMotor.move_voltage(static_cast<std::int32_t>(pct * 12000.0));
	}
